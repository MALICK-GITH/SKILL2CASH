...
import { analyzeMatchScreenshot, compareImageFingerprints, detectPlayersFromText, extractScoreCandidates, normalizeScore, shouldAutoApproveWithOcr } from './ocrService.js';
...
test('shouldAutoApproveWithOcr tolerates a partial OCR read when the evidence stays coherent', () => {
  const duel = {
    ocrScorePlayer1: '2-0',
    ocrScorePlayer2: '',
    ocrPlayersDetectedPlayer1: ['PLAYER ONE'],
    ocrPlayersDetectedPlayer2: [],
    ocrConfidencePlayer1: 70,
    ocrConfidencePlayer2: 68,
    resultPlayer1: { score: '2-0', declaredWinner: 'player-1', imageFingerprint: 'f0f0f0f0f0f0f0f0' },
    resultPlayer2: { score: '2-0', declaredWinner: 'player-1', imageFingerprint: 'f0f0f0f0f0f0f0f1' }
  };
  ...
  assert.match(validation.reason, /revue admin rapide|Confiance OCR insuffisante/i);
});

test('compareImageFingerprints returns a stable similarity ratio', () => {
  assert.equal(compareImageFingerprints('ffffffffffffffff', 'ffffffffffffffff'), 1);
  assert.ok(compareImageFingerprints('ffffffffffffffff', 'fffffffeffffffff') > 0.9);
});

test('analyzeMatchScreenshot rejects oversized screenshots early', async () => {
  const oversized = `data:image/png;base64,${'A'.repeat(2_000_000)}`;
  const result = await analyzeMatchScreenshot(oversized, []);
  assert.equal(result.status, 'failed');
  assert.match(result.error, /1\s*Mo/i);
});
