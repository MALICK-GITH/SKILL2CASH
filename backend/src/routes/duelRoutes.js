...
import { notifyRoom } from '../services/notificationService.js';
...
duelRouter.get('/', asyncHandler(async (req, res) => {
  ...
  const duels = await Duel.find(filter).populate('player1 player2 winner loser', 'username avatar country rank').sort({ createdAt: -1 });
  res.json({ duels });
}));

duelRouter.get('/:id', asyncHandler(async (req, res) => {
  const duel = await Duel.findById(req.params.id).populate('player1 player2 winner loser', 'username avatar country rank');
